...
public class FactorialTest {
	...
	/*@Test : 테스트 메소드에 붙이는 어노테이션
    	...
    	제한사항
     	...
     	테스트 메소드 통화조건
     		...
     		2. assert 구문이 있을 경우 assert구문이 전부 통과 했을 때(참으로 인정 되었을 때)
     		
     	@Before : 초기화 메소드에 붙이는 어노테이션
     	          @Test어노테이션이 붙은 테스트 메소드를 호출하기 전에 먼저 실행됨
     	          @Before ==> @Test(test)
     	          @Before ==> @Test(test2)
     	          @Before ==> @Test(test3)
     	...
     	@BeforeClass, @AfterClass
     	 클래스 단위로 최초 1회로 호출 되는 메소드
     	 @BeforeClass 
     	     @Before ==> @Test(test) ==> @After
     	     @Before ==> @Test(test2) ==> @After
     	     @Before ==> @Test(test3) ==> @After
     	 @AfterClass
     	 제한사항 : 메소드를 static으로 선언해야함
     	           ==> 해당 메소드에서 사용하는 필드는 static 필드여야함 ==> 사용빈도가 떨어지는 원인
     */
	...
	public void test() {
		logger.debug("test()");
		//현재 테스트 환경에서 주어진 조건을 기술
		/***Given***/

		//테스트를 검증하고자 하는 행위(method call)
		/***When***/
		int result = factorial.calculate(5);
		
		//코드를 실행한 결과와 예상한 값이 일치하는지 검증
		/***Then***/
